...
function filtrarTabela() {
    let texto = document.getElementById("barraPesquisa").value.toLowerCase();
    let linhas = document.querySelectorAll("#userTableBody tr");
...
function atualizarTabela() {
  ...
  for (let i = 0; i < usuarios.length; i++) {
    ...
    colunaAdmin.innerText = usuario.ADMIN ? "Sim" : "Não";
...
function salvarUsuario() {
  ...
  const adminCampo = document.getElementById("ADMIN").checked;
...
function editarUsuario(id) {
  ...
  for (let i = 0; i < usuarios.length; i++) {
    if (usuarios[i].id == id) {
      ...
      document.getElementById("ADMIN").checked = usuarios[i].ADMIN;
      break;
    }
  }
}
